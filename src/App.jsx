...
const MM_PER_PX = pdfWidth / 1200;

// ── 動態量測議程目錄，計算統一縮放比例 ──
// 目標：讓議程目錄剛好填滿一頁 A4（不含 margin），
// 後續所有 topic block 等比縮放到相同寬度
let agendaPageScale = 1.0; // 預設 1:1

const agendaSection = target.querySelector('[data-export-section="agenda"]');
if (agendaSection && exportSelection.agenda) {
  const agendaWrapper = document.createElement('div');
  agendaWrapper.style.cssText = `position:fixed;top:-99999px;left:-99999px;width:1200px;background:#F8FAFC;pointer-events:none;`;
  const agendaClone = agendaSection.cloneNode(true);
  disableAnimations(agendaClone);
  agendaWrapper.appendChild(agendaClone);
  document.body.appendChild(agendaWrapper);
  await new Promise(r => setTimeout(r, 100));

  let agendaMeasureCanvas;
  try {
    agendaMeasureCanvas = await window.html2canvas(agendaWrapper, {
      scale: 2, useCORS: true, allowTaint: true,
      backgroundColor: '#F8FAFC', windowWidth: 1200, logging: false,
    });
  } finally {
    document.body.removeChild(agendaWrapper);
  }

  if (agendaMeasureCanvas && agendaMeasureCanvas.width > 0) {
    // canvas.height / 2 = 實際像素高；對應 A4 可用高度 (pdfHeight - margin*2)
    const agendaRealHeightPx = agendaMeasureCanvas.height / 2;
    const agendaRealWidthPx = agendaMeasureCanvas.width / 2;
    
    // 寬度縮放：讓寬度填滿 A4 可用寬
    const scaleByWidth = pdfWidth / (agendaRealWidthPx * MM_PER_PX);
    // 高度縮放：讓高度填滿 A4 可用高
    const scaleByHeight = (pdfHeight - margin * 2) / (agendaRealHeightPx * MM_PER_PX);
    
    // 取較小者，確保完整顯示在一頁內（不裁切）
    agendaPageScale = Math.min(scaleByWidth, scaleByHeight);
  }
}
        ...
        for (let i = 0; i < blocks.length; i++) {
          ...
          const scaledWidth = (canvas.width / 2) * MM_PER_PX * agendaPageScale;
const scaledHeight = (canvas.height / 2) * MM_PER_PX * agendaPageScale;
      ...
      {exportSelection.cover && (
        <div data-export-section="cover" data-pdf-block="true" data-pdf-full-page="true" className="w-full flex flex-row items-center justify-between px-20 bg-[#0A0F1C] border-b-[24px] border-[#B89F5D] relative overflow-hidden" style={{ height: "1697px" }}>
          ...
          <div className="w-[55%] flex flex-col justify-center text-white relative z-10">
            ...
            {config.cover?.desc && (
              <p className="text-3xl text-slate-300 mb-16 leading-[1.8] font-medium border-l-4 border-[#338F88] pl-8 text-left">
                {config.cover?.desc}
              </p>
            )}
            <div className="flex gap-16 py-10 border-t border-white/10 w-full max-w-[700px]">
              <div className="flex flex-col">
                <span className="text-lg text-slate-500 font-bold uppercase tracking-[0.2em] mb-3">Meeting Date</span>
                <span className="text-2xl font-bold text-slate-200 flex items-center gap-3"><Calendar className="w-6 h-6 text-[#B89F5D]" /> {config.sessionDate || "TBD"}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-lg text-slate-500 font-bold uppercase tracking-[0.2em] mb-3">Attendees</span>
                <span className="text-2xl font-bold text-slate-200 flex items-center gap-3"><Users className="w-6 h-6 text-[#B89F5D]" /> {getAttendeePreview(config.attendees)}</span>
              </div>
            </div>
          </div>
                  ...
                  <div key={`agenda-${t.id}`} className={`px-16 py-8 flex gap-10 items-start ${idx !== arr.length - 1 ? 'border-b border-slate-100' : ''}`}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <span className={`px-4 py-1.5 rounded-lg text-sm font-bold ${t.status === "resolved" ? "bg-[#338F88]/10 text-[#338F88]" : "bg-slate-100 text-slate-500"}`}>
